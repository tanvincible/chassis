...
fn test_insert_and_retrieve() {
    ...
    // Insert a vector
    let vector = vec![1.0, 2.0, 3.0].into_iter().cycle().take(128).collect::<Vec<_>>();
...
fn test_exclusive_locking() {
    ...
    assert!(storage2.unwrap_err().to_string().contains("already open"));
}
...
fn test_file_growth_is_page_aligned() {
    ...
    assert_eq!(meta.len() % 4096, 0, "File size must be page-aligned (4KB)");
}
...
fn test_get_vector_slice_correct_length() {
    ...
    for dims in dimensions {
        ...
        let slice = storage.get_vector_slice(0).unwrap();
        assert_eq!(slice.len(), dims as usize, "Slice length mismatch for {} dimensions", dims);
...
fn test_get_vector_slice_alignment() {
    ...
    for i in 0..10 {
        ...
        assert_eq!(ptr % 4, 0, "Slice at index {} has misaligned pointer: 0x{:x}", i, ptr);
    }
}
...
fn test_get_vector_slice_distance_calculation() {
    ...
    assert!(
        ...
        "Distance calculation incorrect: expected {}, got {}",
        expected,
        distance
    );
}
